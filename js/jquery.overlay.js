...
(function($) {
    $.fn.overlay = function(options) {
        ...
        return this.each(function(evt) {
            var this_obj = $(this);
            if (! this_obj.hasClass('overlay-trigger')) {
  ...
  function create($src, opts) {
    ...
    .css({
        ...
        width: opts.container === 'body' ? '100%' : $(opts.container).outerWidth(),
        height: opts.container === 'body' ? '100%' : $(opts.container).outerHeight(),
        position: 'absolute',
        zIndex: 9999,
        //display: 'none',
        overflow: 'hidden'
